...
set serveroutput on
declare
    cursor c is select * from course natural join (select course_id, count(*) as tot from takes group by course_id);
begin
    for co in c
    loop
        dbms_output.put_line('ID: '||co.course_id||' Title: '||co.title||' Dept: '||co.dept_name||' Credits: '||co.credits||' Total: '||co.tot);
    end loop;
end;
/